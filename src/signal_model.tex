    ...
    \begin{table}[tbh]
       \begin{center}
           ...
           \toprule
               ...
               507684  &	VBF \kl=-5, \kvv=1, \kv=0.5 &  MadGraph+Pythia8 &	4.45    & $200000  +260000  +340000  $\tnote{a}\\
           \bottomrule
           \end{tabular}
           \begin{tablenotes}
...
\section{Signal Combination} \label{sec:signal_combination}
    ...
    The process of combining a few samples in such a way as to model the entire parameter space of scaling factors is based on exploiting the underlying mathematics of the differential cross-section formula.
    ...
    \begin{equation}
        \vec{\xsec} = \minimatrix{ \dxsecMN{1} \\ \dxsecMN{2} \\ \dxsecMN{3} \\ \dxsecMN{4} \\ \dxsecMN{5} \\ \dxsecMN{6} }
        \qquad,\qquad
    ...
    \newcommand{\combterm}[1]{ g_{#1} \frac{d\sigma_{#1}}{d \mhh} }
        ...
        producing a final expression for the differenctial cross-section in the form
    \begin{equation} \begin{split}
        \dXsecM &= \vec{f}(\kvv,\kl,\kv) \; F^{-1} \; \vec{\xsec} \\
        \dXsecM &= \vec{g}(\kvv,\kl,\kv) \bullet \vec{\xsec}
            \quad,\quad \vec{g} \equiv \vec{f}(\kvv,\kl,\kv) \; F^{-1} \\
        \dXsecM &= 
            \combterm{1} +
            \combterm{2} +
            \combterm{3} +
    ...
    In practice, the cross-section values of these six \textit{basis} samples are represented by their yields from MC simulation,
        binned by their \mhh, which are added together after being multiplied by their respective combination coefficient $g_i$
...
\section{Solidarity} \label{sec:solidarity}
    ...
    The formula for the combination of the samples amounts to weighting each sample's yield by a scale-factor-dependent coefficient,
    ...
    As well, the scale-factor-based coefficients can vary dramatically across the scale-factor space.
    The product of the sample yield and the scale-factor coefficient (the \textit{weighted yield})
        can therefore fluctuate a great deal across the scale-factor space.
    For many regions of the scale-factor space, it can happen that one or two of the samples' weighted yields vastly outweigh the others.
    ...
    Due to the nature of the weighted yields,
        ...
        and on the scale-factor coefficient ($c_i(\kvv,\kl,\kv)$) of the specific point in the scale-factor space to be modeled.
    ...
    As the metric is calculated on a per-scale-factor basis,
