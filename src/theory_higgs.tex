...
\section{Breaking Symmetry: GWS Theory}
    ...
    \begin{equation} \begin{split}
        ...
        \Lag & = \frac{1}{2} \left|D_{\mu} \minimatrix{0\\h(x)+v}\right|^2
         ...
         & = \Lag_h + \Lag_v
    \end{split} \end{equation}
    Where $\Lag_h$ takes a form similar to Eq. \ref{eq:basic_higgs}, incorporating both the $h$ and $h$/$v$ cross terms.
    ...
    Returning finally to Eq. \ref{eq:lagv}, I now have
    \begin{equation} \begin{split}
        \label{eq:lagv_full}
...
\section{Keystone of the Standard Model: The Higgs Boson} \label{sec:higgs_boson}
    ...
    With the critical role of the Higgs Field established, it is now time to return to Eq. \ref{eq:fullHiggs},
    ...
    Starting with $\Lag_D$, (and defining $Q_{\mu}$ as the matrix from Eq. \ref{eq:electroweak_matrix})
    ...
    The $(h+v)^2$ term will produce couplings quadratic in $v$ ($\Lag_v$ of Eq. \ref{eq:lagv_full}), and both linear and quadratic in $h$.
    ...
    If they exist at all, there is no guarantee that these interactions have the coupling strength indicated by Eq. \ref{eq:higgsfull}.
