...
\section{Data-Driven Background Estimate}
    ...
    Indeed, based on the relative rates of QCD multijet events and the SM \vbfhhproc process (see Table \ref{tab:mcyields}),
        most of the events remaining in the data are very likely background.
    [TODO: steve, why is it so hard to find anything that actually states what the rough event yield for multijet is?
        Every single paper I've found just states as fact, without any citation, that multijet events are this overwhelming background,
        without ever quantifying the statement D:< Got any tips? Or is this just a case of ``try harder''?]
    ...
    Unfortunately, observation of the Control Regions shows that the kinematics of the 2b and 4b data are not expected to match completely.
...
\section{Neural Network Training and Uncertainty} \label{sec:nn_training}
    ...
    A neural network (NN) is trained to identify how to reweight 2b data such that the cumulative kinematic distribution looks like that of 4b data.
...
\section{VBF Background Reweighting Variables} \label{sec:vbf_bgdNNRW}
    ...
    To measure how strongly related any two variables are I used the Pearson correlation coefficient,
        which is a scalar metric of how closely the variables follow a linear correlation.
