...
\section{Statistical Mathematics}
    ...
    \begin{figure}
        ...
        \caption{
            A plot of the probability distribution function (PDF)
                and cumulative PDF for the estimated background and
                two variations (\kvv=1 and \kvv=3) of the signal hypothesis,
                with the observed yield denoted in both.
            In the \kvv=1 case, the signal yield is so small that the ``background only''
                and ``signal plus background'' distributions overlap almost completely.
            For \kvv=3 there a much greater distinction between these distributions,
                to the extent that the observed event yield now lies at the extreme left edge of the S+B distribution.
        }
    \end{figure}
    \FloatBarrier
    ...
    In contrast the plot for \kvv = 3, which highlights a $\mu$ of 0.91,
        indicates that the signal would have to be \textit{reduced} to 91\% of its yield
        before it could be considered as reasonably compatible with data.
    ...
    Shown in Figs. \ref{fig:xseclimits_kvv} and \ref{fig:xseclimits_kl},
        these scans indicate how far the \vbfproc cross-section would have to deviated from
...
\section{VBF HH 4b Limit-Setting Framework}
    ...
    Although a wealth of information can be obtained from event yields alone,
        far more statistical power can be wrought by making full use of
    ...
    By splitting the event count up across a number of different
    ...
    Thus, even though there are vastly more background events overall,
        ...
        in which there are a comparable or even dominant number of signal events.
    ...
    There is only one parameter of interest ($\mu$), but there are now six total parameters of which $L$ is a function. %FIXME mention the bootstrap uncertainty too
    ...
    \begin{figure}
        \centering
        \begin{subfigure}{0.48\textwidth} 
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/k2v_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_kl_1.0_mu}
            \caption{\kvv $\mu$ Scan}
            \label{fig:mulimits_kvv_rooFit}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth}
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/kl_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_k2v_1.0_mu}
            \caption{\kl $\mu$ Scan}
            \label{fig:mulimits_kl_rooFit}
        \end{subfigure}\\
        \begin{subfigure}{0.48\textwidth} 
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/k2v_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_kl_1.0_xs}
            \caption{\kvv Cross-section Scan}
            \label{fig:xseclimits_kvv_rooFit}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth}
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/kl_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_k2v_1.0_xs}
    ...
    \begin{figure}
        \centering
        \begin{subfigure}{0.48\textwidth} 
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/2D_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_k1v1.0_exclusion}
            \caption{2D \kvv/\kl Limits, for \kv=1}
            \label{fig:limit_slice_kv_1p0_rooFit}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth} 
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/2D_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_k1v-1.0_exclusion}
            \caption{\kvv/\kl Limits, for \kv=-1}
            \label{fig:limit_slice_kv_1p0_rooFit}
        \end{subfigure}\\

        \begin{subfigure}{0.48\textwidth}
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/2D_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_kl1.0_exclusion}
            \caption{\kvv/\kv Limits, for \kl=1}
            \label{fig:limit_slice_kl_1p0_rooFit}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth} 
            \includegraphics[width=\linewidth,height=\textheight,keepaspectratio]{results/2D_scan_thesis_3D_samps_vbf_pd_vbf_inc161718_k2v1.0_exclusion}
            \caption{\kl/\kv Limits, for \kvv=1}
            \label{fig:limit_slice_k2v_1p0_rooFit}
        \end{subfigure}
        \caption{
            ...
            The \kl/\kv limit at \kvv=1 looks different simply because \kvv is better constrained
                [Steve, should I show more of these slices for both this and the earlier yield-based version?
                    If I look at the right \kvv slice I should have a yield-based analog to the slice shown here].
            The \kvv/\kv limit here is restricted between -1 and 2 [TODO extend to 2] due to computational constraints.
        }
    \end{figure}
