...
\section{Statistical Mathematics}
    ...
    For example, in the case where the oberved number of events is far larger than the background estimate,
    ...
    The total yields of the data, Background estimate, and the SM and \kvv=3 signal hypotheses are provided in Table \ref{tab:event_yield},
        and their PDF and cumulative PDFs can be seen in Fig. \ref{fig:poisson_sig:pdf} and \ref{fig:poisson_sig:Cpdf}.
    ...
    The complete lack of any noticeable deviation from the background-only hypothesis (Fig. TODO)
        thus results in the low signal p-value of 0.03.
    ...
    Figures TODO show such a test for \kvv set to 1 and 3.
    ...
    Utilizing the signal model linear combination equation from Section \ref{sec:signal_combination},
        the signal hypothesis for any value of \kvv can be tested, producing Fig. TODO.
    A similar plot can be produced by scanning across the values of \kl (Fig. TODO).
    ...
    Shown in Figs TODO, these scans indicate how far the \vbfproc cross-section would have to deviated from
    ...
    The two-dimensional $\kappa$ limits can be seen in Figs TODO,
