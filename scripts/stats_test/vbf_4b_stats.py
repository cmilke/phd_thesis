...
def make_sb_poisson_plots(results=None, prefix='', couplings=None):
    ...
    ax.fill_between(range(0,observed), 0, poisson_bgd_values[:observed], color='blue', hatch='///', alpha=0.3, label=f'bgd p-value={bgd_pvalue:.2f}')
    ax.fill_between(range(0,observed), 0, poisson_bs_values[:observed], color='red', hatch='///', alpha=0.3, label=f'S+B p-value={bs_pvalue:.2f}')
